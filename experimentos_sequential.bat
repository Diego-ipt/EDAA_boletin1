...
echo =====================================================
echo EXPERIMENTOS PARA ANALISIS DE BUSQUEDA SECUENCIAL
echo =====================================================
echo.
...
echo Tamano,Tiempo,PosicionEncontrada > resultados\exp1_tam_seq.csv
...
for %%s in (1024 2048 4096 8192 16384 32768 65536 131072 262144 524288 1048576) do (
    ...
    for /f "tokens=1,2" %%a in ('sequentialS.exe data/data_size_%%s.txt 3') do (
        echo %%s,%%a,%%b >> resultados\exp1_tam_seq.csv
    )
)
...
echo PosicionReal,Tiempo,PairUsado > resultados\exp2_pos_seq.csv
...
for /L %%i in (0,1,9) do (
    ...
    for /f "tokens=1,2" %%a in ('sequentialS.exe data/data_size_1048576.txt %%i') do (
        echo %%b,%%a,%%i >> resultados\exp2_pos_seq.csv
    )
)
...
echo - resultados\exp1_tam_seq.csv
echo - resultados\exp2_pos_seq.csv
echo.
...
echo.
echo NOTA: La busqueda secuencial tiene complejidad O(n)
echo      Los tiempos seran mayores que la busqueda binaria
echo.