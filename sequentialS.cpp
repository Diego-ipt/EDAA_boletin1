...
#include <sstream>
#include <ctime>
#include "search/sequential_search.cpp"
...
int main(int argc, char* argv[]) {
    ...
    // Perform sequential search with timing using multiple iterations
    std::int64_t result = -1;
    const int iterations = 10000; // Fewer iterations since sequential is slower
    
    auto start = std::chrono::high_resolution_clock::now();
    for (int i = 0; i < iterations; i++) {
        result = sequential_search(data.data(), size, target_value);
    }
